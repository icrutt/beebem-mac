...
/*
  ...
  static struct timeval previous,now;
  static struct timeval fprevious,fnow;
  static struct timezone tpz;
  ...
  if (!Have_GotTime) {
    gettimeofday(&previous, &tpz);
    gettimeofday(&fprevious, &tpz);
    Time_FrameCount=-1;
    Have_GotTime=1;
  };
  ...
  if (Time_FrameCount==(BEEB_DOTIME_SAMPLESIZE-1)) {
    ...
    frametime=(now.tv_sec * 1000000L + now.tv_usec) - (previous.tv_sec * 1000000 + previous.tv_usec);
//	if (now.tv_usec < previous.tv_usec) frametime += 1000000; 
...
//    cerr << "Frametime: " << frametime << "s fps = " << (1/frametime) << " Total cycles = " << TotalCycles << " Cycles in last unit = " << (TotalCycles-OldCycles) << "\n";
    ...
    gettimeofday(&fnow, &tpz);

    frametime=(fnow.tv_sec * 1000000L + fnow.tv_usec) - (fprevious.tv_sec * 1000000 + fprevious.tv_usec);
...
//    cerr << "Frametime for 50 frames is : " << frametime << "s fps=" << (1/frametime) << " Total cycles=" << TotalCycles << " Cycles in last unit=" << (TotalCycles-OldCycles) << "\n";
    ...
    if (frametime < 0.02)
	{
		us = (long) ((0.02 - frametime) * 1000000.0);
		usleep( us + 500);
	}
...
void VideoDoScanLine(void) {
  ...
  if (VideoState.IsTeletext) {
    ...
    if ( (VideoState.CharLine!=-1) ||
	    ( (VideoState.CharLine == -1) && 
		CRTC_VerticalDisplayed > CRTC_VerticalTotal)) {
...
void VideoAddLEDs(void) {
	...
	if (LEDs.ShowDisc)  {
		mainWin->doLED(578,LEDs.HDisc[0]);
		mainWin->doLED(588,LEDs.HDisc[1]);
		mainWin->doLED(598,LEDs.HDisc[2]);
		mainWin->doLED(608,LEDs.HDisc[3]);
